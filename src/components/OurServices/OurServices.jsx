...
export default function OurServices() {
  return (
    <div className={styles['our-services']}>
      <p className={styles['sub-para-text']}>Work with people who think differently</p>
      <h2 className={styles['head-text']}>Explore our superpowers</h2>
        ...
        {services.map((service) => (
          <div key={service.id} className={styles['service-card']}>
            ...
            <div className={styles['service-content']}>
              <h3 className={styles['sub-head-text']}>{service.title}</h3>
              <p className={styles['sub-para-text']}>{service.description}</p>
              <button className={styles['transparent-bg-btn']}>
                Learn More <GoArrowRight className={styles['arrow-icon']} />
              </button>
            </div>
          </div>
        ))}
      </div>
      <div className={styles['ourservices-btn-container']}>
        <a href="/" className={styles['white-bg-btn']} style={{ marginTop: "50px" }}>
          View All Services <GoArrowRight className={styles['arrow-icon']} />
        </a>
      </div>
    </div>
  );
