...
import styles from "./Clients-img.module.css";
...
export default function ClientsImg() {
  ...
  return (
    <section className={styles['Clients-img-section']}>
      <div className={styles['Clients-section-head-container']}>
        <h2 className={styles['head-text']}>Clients</h2>
      </div>

      <div className={styles['clients-rows-container']}>
        {rows.map((rowLogos, rowIndex) => (
          <div key={rowIndex} className={styles['clients-row-swiper-wrapper']}>
            <Swiper
              slidesPerView={1}
              spaceBetween={0}
              loop={true}
              ...
              modules={[Autoplay]}
              className={styles['clients-row-swiper']}
            >
              {rowLogos.map((logo, index) => (
                <SwiperSlide key={index} className={styles['client-slide']}>
                  <div className={styles['Clients-img-wrapper']}>
                    <img
                      src={logo}
                      alt={`Client logo ${index + 1}`}
                      className={styles['Clients-img']}
                    />
                  </div>
                </SwiperSlide>
              ))}
