...
const HomeCarosule = () => {
  ...
  ---------------------------------- */
  return (
    <div className="wac-video-carousel">
      <div ref={overlayRef} className="whole-overlay" />

      {/* VIDEOS */}
      <div className="wac-carousel-track">
        {slides.map((slide, index) => (
          <div
            key={index}
            className={`wac-slide ${index === currentIndex ? "active" : ""}`}
          >
            <video
              ref={videoRefs.current[index]}
              className="wac-video-bg"
              muted
              playsInline
              preload="auto"
            >
      ...
      {/* LEFT CONTENT */}
      <div className="wac-overlay">
        <div className="wac-content left-align">
          <h2 className="wac-heading">{slides[currentIndex].heading}</h2>
          <h3 className="wac-subheading">{slides[currentIndex].subheading}</h3>
          <p className="wac-description">{slides[currentIndex].description}</p>

          <a href={slides[currentIndex].link} className="wac-cta-button">
      ...
      {/* RIGHT CTA + PROGRESS */}
      <div className="wac-right-cta">
        <div className="wac-thumbnail">
          <img
        ...
        <div className="wac-cta-details">
          <div>
            <span className="wac-next">Next</span>
            <h3 className="wac-title">
              {slides[(currentIndex + 1) % slides.length].title}
            </h3>
          </div>

          <div className="wac-pagination">
            {slides.map((_, index) => (
              <div
                key={index}
                ref={progressBarRefs.current[index]}
                className="wac-progress-bar"
